...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (nameData.data.is_baby) {
            ...
            return res.json({"id":dataFrontEnd.data.id, "name": dataFrontEnd.data.name, "sprite": data.data.sprites.front_default, "types": arr, "bgColor": colorData.data.color.name, "evolution": data.data.chain.evolves_to[0].evolves_to[0].species.name});
        }
    }
    catch(error) {
        console.log(error);
