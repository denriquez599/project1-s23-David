...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (nameData.data.is_baby) {
            return res.json({"id":data.data.id, "sprite": data.data.sprites.front_default, "types": arr, "bgColor": colorData.data.color.name, "evolution": dataURL.data.chain.evolves_to[0].species.name});
        } else {
